...
import { COLLECTION_LABEL_NAME, FROM_ANNOTATION_REL_TYPE, TO_ANNOTATION_REL_TYPE } from '../constants';
...
export class CollectionService {
  ...
  async getFilterable(): Promise<Record<string, CollectionNameDto[]>> {
    ...
    const filterableCollections = guidelines.scenarios.findByCollection.filterable;
  ...
  async getCollectionNamesOfType(collectionType: string, parentId?: string): Promise<CollectionNameDto[]> {
    const guidelines = await this.guidelinesService.get();
    ...
    let pattern = new Cypher.Pattern(col, {
      labels: [guidelines.collection.metaType, new Cypher.Param(collectionType)],
    });
    ...
    const res = await this.neo4jService.read<{ id: string; label: string }>(cypher, params);
  ...
  async getCollectionsOfEntityNode(entityNode: Cypher.Node, query: Cypher.With): Promise<[With, Cypher.Variable]> {
    const guidelines = await this.guidelinesService.get();
    ...
    const withEntityC = new Cypher.With(entityNode, c).where(
      Cypher.or(
        Cypher.isNull(c),
        Cypher.any(lx, Cypher.labels(c), Cypher.in(lx, new Cypher.Param(guidelines.scenarios.searchEntities.shownCollections))),
      ),
    );
  ...
  async getCollectionsOfEntity(entityId: string): Promise<CollectionNameDto[]> {
    ...
    const matchEntity = new Cypher.Match(
      new Cypher.Pattern(entity).where(Cypher.eq(entity.property(eIdLabel), new Cypher.Param(entityId))),
    );
    ...
    const returnClause = new Cypher.Return([col.property(colIdLabel), 'id'], [col.property(colNameLabel), 'label']).distinct();

    const clause = matchEntity.match(matchCollections).optionalMatch(optionalParents).return(returnClause);
